...
## 이미지 불러오기
heart_img = img_read("heart", 0.03)
angel_img = img_read("angel", 0.15)
...
point_font = pygame.font.Font("/Users/mzc01-nkoe/Library/Fonts/BMDOHYEON_otf.otf", 50)
finalp_font = pygame.font.Font("/Users/mzc01-nkoe/Library/Fonts/BMDOHYEON_otf.otf", 80)
ready_font = pygame.font.Font("/Users/mzc01-nkoe/Library/Fonts/BMDOHYEON_otf.otf", 80)
ready2_font = pygame.font.Font("/Users/mzc01-nkoe/Library/Fonts/BMDOHYEON_otf.otf", 20)
...
while not exit:
    ...
    while not exit:
        ...
        if game_over == False:
            total_time = now_time - game_start_time + extra_time ## extra_time 변수 추가
        # 새 생성
        ...
        ## 천사 생성
        if random.random() < 0.01:  # 천사가 나올 확률 (0.01 = 1%)
            aa = angel()
            angel_list.append(aa)
        ...
        for dd in dd_list:
            ...
            X, Y = player.pos
            W, H = player.size
            dung_x, dung_y = dd.pos
            dung_w, dung_h = dd.size
            if X-dung_w< dung_x and dung_x<X+W and Y-dung_h<dung_y and dung_y<Y+H-dung_h and game_over == False:
                game_over = True
                sound_sad.play()
        ...
        ## 천사 움직임
        for aa in angel_list:
            aa.timer += aa.ani_move
            aa.pos = (aa.pos[0] + aa.move_x, aa.pos[1] + aa.move_y)
            aa.show
            
            #하트 드롭
            if aa.move_x > 0: ##왼쪽에서 오른쪽
                if aa.pos[0] >= aa.drop_x and not aa.drop:
                    hh = heart(round(aa.pos[0] + aa.size[0] /2), aa.pos[1] + aa.size[1], aa.move_x)
                    heart_list.append(hh)
                    aa.drop = True
            else: ##오른쪽에서 왼쪽
                if aa.pos[0] <= aa.drop_x and not aa.drop:
                    hh = heart(round(aa.pos[0] + aa.size[0]/2), aa.pos[1] + aa.size[1], aa.move_x)
        ...
        ## 하트 움직임
        for hh in heart_list:
            hh.move_y += hh.a_y
            hh.pos = (hh.pos[0] + hh.move_x, hh.pos[1] + hh.move_y)
            X, Y = player.pos
            W, H = player.size
            heart_x, heart_y = hh.pos
            heart_w, heart_h = hh.size
            if X-heart_w< heart_x and heart_x<X+W and Y-heart_h<heart_y and heart_y<Y+H-heart_h and game_over == False:
                print("Heart!") #디버깅용
                extra_time += 10000
                heart_list.remove(hh)
        ...
        ## 천사 소멸
        del_list = []
        for i, aa in enumerate(angel_list):
            if aa.pos[0] < -aa.size[0] or aa.pos[0] > size[0]:  # 천사가 화면 밖으로 나갔을 때
                del_list.append(i)
        del_list.reverse()
        for i in del_list:
            del angel_list[i]
        ...
        ## 하트 소멸
        del_list = []
        for i, hh in enumerate(heart_list):
            if hh.pos[1] > size[1] - hh.size[1]:  # 하트가 화면 아래로 나갔을 때
                del_list.append(i)
        del_list.reverse()
        for i in del_list:
            del heart_list[i]
